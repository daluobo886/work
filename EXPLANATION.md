...
## 策略与图表补充
- **不仲裁场景**：S1/S2/S5/S6 以及新增场景默认 `allowArbitration = false`，全程无一次性补偿也不回补工资差，风险指标（EV/方差/CE/溢价）均置 0，仅保留折现现金流路径。
- **Monte Carlo 风险定义**：每个场景运行 5,000 次模拟，扰动贴现率并对仲裁场景抽样赔付倍数与执行率，得到 NPV 分布；风险-收益散点的横轴为均值 E[NPV]，纵轴为标准差 σ(NPV)。
- **热力图**：用当前参数的法定 N、赔付倍数档与五档执行率组合生成“仲裁结果 vs 执行率”矩阵，展示理论可执行金额的大小，与具体场景是否仲裁无关。
- **敏感度龙卷风**：对当前选中场景的五个关键参数（贴现率、法律胜率、执行率、再就业收入、管理员上限）做 ±20% 单因素冲击，重算 NPV，取绝对值较大的方向作为条形图高度。
